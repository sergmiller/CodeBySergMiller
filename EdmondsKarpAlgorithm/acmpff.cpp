...
int main() {
    ...
    for(int i = 0;i < n; ++i)
    {
        for(int j = 0;j < m;++j)
        {
            ...
            if(x == -1)
            {
                edges.push_back(make_pair(make_pair(i,n + j), x));
            }
            else
            {
                row[i] -= x;
    ...
    for(int i = 0;i < n;++i)
    {
        if(row[i] < 0)
            f = 1;
        edges.push_back(make_pair(make_pair(n + m,i), row[i]));
    }
    ...
    for(int j = 0;j < m;++j)
    {
        if(col[j] < 0)
            f = 1;
        edges.push_back(make_pair(make_pair(n + j,n + m + 1), col[j]));
    }
    ...
    Graph graph(n + m + 2, edges);
    
    Network net(&graph, n + m, n + m + 1);
